...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private const val DATABASE_NAME = "bin_database"
...
//                    .createFromAsset("database/${DATABASE_NAME}.db")
        ...
        fun initDatabase(instance: AppDatabase) {
            ...
            // First, add all the default BinColours, so we can fetch their bcId (autogenerated)
            ...
            val landfillColor = fromColorToLong(BinColours.LANDFILL)
            val recyclingColor = fromColorToLong(BinColours.RECYCLING)
            val gardenColor = fromColorToLong(BinColours.GARDEN)
            landfillBin.binColoursId = colDao.getByPrimary(landfillColor).first().bcId
            recyclingBin.binColoursId = colDao.getByPrimary(recyclingColor).first().bcId
            gardenBin.binColoursId = colDao.getByPrimary(gardenColor).first().bcId
